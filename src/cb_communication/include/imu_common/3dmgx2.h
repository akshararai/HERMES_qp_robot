...
namespace microstrain_3dmgx2_imu
{
  ...
  class IMU
  {
    ...
    static const int MAX_BYTES_SKIPPED  = 100000;
  ...
  public: 
    ...
    //! Gravity (m/sec^2)
    static const double GRAV_CONST               = 9.80665;
    static const int DATA_RATE_DECIM 	= 4;
    ...
    bool setCommunicationSettings();
    bool setSamplingSettings();

    bool turnOffRealignment();
    bool realignVectors(uint8_t up_duration, uint8_t north_duration);
     ...
     */
    void receiveAccelAngrate(uint64_t *time, double accel[3], double angrate[3], uint32_t* timestamp);
    ...
    //! Transact a message of type "ACCEL_ANGRATE_MAG"
	/*!
	 * \param time    Pointer to uint64_t which will receive time
	 * \param accel   array of accelerations which will be filled
	 * \param angrate array of angular rates which will be filled
	 * \param mag     array of magnetometer orientations which will be filled
	 */
    void transactStabAccAngrateStabMag(uint64_t *time, double accel[3], double angrate[3], double mag[3]);

    //! Transact integrated velocity and orientation
	/*!
	 * \param time    Pointer to uint64_t which will receive time
	 * \param delta_angle   array of velocities which will be filled
	 * \param delta_velocity array of angles which will be filled
	 */
    void transactDeltaAngleDeltaVel(uint64_t *time, double delta_angle[3], double delta_velocity[3]);

    //! Transact a message of type "ACCEL_ANGRATE_ORIENTATION"
    /*!
     * \param time        Pointer to uint64_t which will receive time
     * \param accel       array of accelerations which will be filled
     * \param angrate     array of angular rates which will be filled
     ...
     */
    void transactAccelAngrateOrientation(uint64_t *time, double accel[3], double angrate[3], double orientation[9], uint32_t* timestamp);
     ...
     * \param orientation orientation matrix which will be filled
     */
    void receiveAccelAngrateOrientation(uint64_t *time, double accel[3], double angrate[3], double orientation[9], uint32_t* timestamp);
	 ...
	 * \param orientation orientation matrix which will be filled
	 */
    void receiveAccelAngrateMagOrientation (uint64_t *time, double accel[3], double angrate[3], double mag[3], double orientation[9], uint32_t* timestamp);
	...
	void receiveQuaternion(uint64_t *time, double quat[4], uint64_t *timestamp);
	uint32_t initTimestamp(void);

	void receiveRawAccelAngrate(uint64_t *time, double accel[3], double angrate[3]);

	uint32_t getFirmwareNumber(void);
  ...
  private:
    ...
    uint32_t toUint32_t(double var);
    ...
    double fixed_offset_, offset_, d_offset_, sum_meas_;
