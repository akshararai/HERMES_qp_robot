...
int m3dmg_sendCommand(int deviceNum, char command, char *response, int responseLength) {
	...
	if(DEBUG) {
		logPrintf("m3dmg_send: port #: %d\n", portNum);
	}

	/* send data out the port */
	status = sendData (portNum, &command, 1);
	if(DEBUG) {
		logPrintf("m3dmg_send: tx status : %d\n", status);
	}
	if(status == M3D_COMM_OK) {
		...
		if(responseLength > 0) {
			...
			if (DEBUG) {
				logPrintf("m3dmg_send: rx status : %d\n", status);
			}
			if(status == M3D_COMM_OK) {
				checkSum = convert2int (&response[responseLength - 2]);
				int calculatedCheckSum = calcChecksum(&response[0], responseLength);
				if(checkSum != calculatedCheckSum) {
					logPrintf("m3dmg_send: ERROR checkSum = %i != %i = calculatedCheckSum\n",checkSum,calculatedCheckSum);
	...
	if(DEBUG) {
		logPrintf("m3dmg_send: final status : %d\n", status);
	}
	return status;
}
...
int m3dmg_openPort(int portNum, int baudrate, int size, int parity, int stopbits) {
	...
	// logPrintf("m3dmg_openPort>> opening port...\n");
	...
	// logPrintf("m3dmg_openPort>> set communications parameters...\n");

	// set communications parameters
	errcheck = setCommParameters(porth, baudrate, size, parity, stopbits);

	// logPrintf("m3dmg_openPort>> errcheck after setCommparameters = %i\n",errcheck);

	if(errcheck != M3D_COMM_OK) {
		return errcheck;
	}

	logPrintf("m3dmg_openPort>> set timeouts\n");
...
int
m3dmg_getSerialNumber (int deviceNum, int *serialNum)
{
...
int
m3dmg_getFirmwareVersion (int deviceNum, char *firmware)
{
...
int
m3dmg_getTemperature (int deviceNum, float *temp)
{
...
int
m3dmg_getTimerSeconds (int deviceNum, float *ticks)
{
	char cmd = CMD_TEMPERATURE;
...
int
m3dmg_getRawSensorOutput (int deviceNum, float mag[], float accel[],
		float angRate[])
{
...
int
m3dmg_getVectors (int deviceNum, float mag[], float accel[], float angRate[],
		int stableOption)
{
	char cmd = CMD_RAW_SENSOR;
	char responseBuffer[128];
	...
	else
	{
		...
		{
			mag[i] =
					(float) convert2short (&responseBuffer[1 + i * 2]) /
					convertFactor;
			accel[i] =
					(float) convert2short (&responseBuffer[7 + i * 2]) /
					convertFactor;
			angRate[i] =
					(float) convert2short (&responseBuffer[13 + i * 2]) /
					angularFactor;
		}
		return M3D_OK;
	}
...
int
m3dmg_getQuaternions (int deviceNum, float q[], int stableOption)
{
	char cmd;
	char responseBuffer[128];
...
int
m3dmg_captureGyroBias (int deviceNum)
{
	char cmd;
	char responseBuffer[128];
	int status;
...
int
m3dmg_getOrientMatrix (int deviceNum, float mx[][3], int stableOption)
{
	char cmd;
	char responseBuffer[128];
	float convertFactor = 8192.0f;
	...
	else
	{
		for (i = 0; i < 3; i++)
		{
			...
			{
				mx[i][j] =
						(float) convert2short (&responseBuffer[1 + 2 * (j * 3 + i)]) /
						convertFactor;
			}
		}
		return M3D_OK;
...
int
m3dmg_getGyroStabQuatVectors (int deviceNum,
		float q[],
		float mag[], float accel[], float angRate[])
{
	...
	else
	{
		...
		{
			mag[i] =
					(float) convert2short (&responseBuffer[9 + i * 2]) /
					convertFactor;
			accel[i] =
					(float) convert2short (&responseBuffer[15 + i * 2]) /
					convertFactor;
			angRate[i] =
					(float) convert2short (&responseBuffer[21 + i * 2]) /
					angularFactor;
		}		return M3D_OK;
	}
}
...
int
m3dmg_getEulerAngles (int deviceNum, float *pitch, float *roll, float *yaw,
		int stableOption)
{
	char cmd;
	char responseBuffer[128];
...
int
m3dmg_getEEPROMValue (int deviceNum, short address, short *value)
{
	int status;
	int portNum = deviceMap[deviceNum];
	char cmdBuffer[5];
 ...
 *--------------------------------------------------------------------------*/
int
m3dmg_setEEPROMValue (int deviceNum, short address, short *value)
{
	int status;
	int portNum = deviceMap[deviceNum];
	short valueCheck;
	...
	if (status == M3D_COMM_OK)
	{
		...
		if (status == M3D_COMM_OK)
		{
			...
			if (DEBUG)
				printf ("eeprom check returned: %d\n", valueCheck);
		}
		else
			status = M3D_COMM_READ_ERROR;
	}
