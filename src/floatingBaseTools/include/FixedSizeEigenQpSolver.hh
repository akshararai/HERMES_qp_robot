 ...
 *********************************************************************
 ...
 \file         FixedSizeEigenQpSolver.hh
...
#ifndef FIXEDSIZEEIGENQPSOLVER_HH_
#define FIXEDSIZEEIGENQPSOLVER_HH_

namespace floating_base_utilities
{

template<int nVars,  int nIneqCon>
class FixedSizeEigenQpSolver
{
public:
  FixedSizeEigenQpSolver();
  virtual ~FixedSizeEigenQpSolver();


  static double solve_quadprog(Eigen::Matrix<double,nVars,nVars> & Hessian,
  ...
  static double solve_quadprog(Eigen::Matrix<double,nVars,nVars> & Hessian,
        Eigen::Matrix<double,nVars,1> & g0,
        const Eigen::Matrix<double,nVars,nIneqCon> & CI, const Eigen::Matrix<double,nIneqCon,1> & ci0,
        Eigen::Matrix<double,nVars,1> & x, Eigen::MatrixBase<invMatDer>& Hessian_factor_inv);
...
private:
  ...
  template<typename invMatDer>
  static inline void compute_d(Eigen::Matrix<double,nVars,1> & d,
      const Eigen::MatrixBase<invMatDer> & Hessian_factor_inv, const Eigen::Matrix<double,nVars,1> & np)
  {
    d = Hessian_factor_inv.adjoint() * np;
  }

  template<typename invMatDer>
  static inline void update_z(Eigen::Matrix<double,nVars,1> & z,
      const Eigen::MatrixBase<invMatDer> & Hessian_factor_inv,
      const Eigen::Matrix<double,nVars,1> & d, int iq)
  {
    z = Hessian_factor_inv.rightCols(z.size()-iq) * d.tail(d.size()-iq);
  }

  static inline void update_r(const Eigen::Matrix<double,nVars,nVars> & R,
      Eigen::Matrix<double,nIneqCon+0,1>& r, const Eigen::Matrix<double,nVars,1>& d,
      int iq)
  {
  ...
  template<typename invMatDer>
  static bool add_constraint(
      Eigen::Matrix<double,nVars,nVars> & R, Eigen::MatrixBase<invMatDer> & Hessian_factor_inv,
      Eigen::Matrix<double,nVars,1> & d,
      int& iq, double& R_norm);

  template<typename invMatDer>
  static void delete_constraint(
      Eigen::Matrix<double,nVars,nVars>& R, Eigen::MatrixBase<invMatDer>& Hessian_factor_inv,
      Eigen::Matrix<int,nIneqCon+0,1> & A, Eigen::Matrix<double,nIneqCon+0,1> & u,
      int p, int& iq, int l);
};
    ...
    Eigen::Matrix<double,nVars,1> & x, Eigen::MatrixBase<invMatDer>& Hessian_factor_inv)
{
  ...
  int p=0;
  int m=nIneqCon;
  Eigen::Matrix<double,nVars,nVars> R;

  Eigen::Matrix<double,nIneqCon+0,1> s, r, u, u_old;
  Eigen::Matrix<double,nVars,1> z, d, np, x_old;
  ...
  Eigen::Matrix<int,nIneqCon+0,1> A, A_old, iai, iaexcl;
  int q;
  int iq, iter = 0;
  ...
  R_norm = 1.0; /* this variable will hold the norm of the matrix R */
    ...
    assert((Hessian*Hessian_factor_inv*Hessian_factor_inv.transpose() -
        Eigen::MatrixXd::Identity(Hessian.rows(), Hessian.cols())).norm() < 0.0001 &&
           "Factored inverse is probably wrong.");
#endif
...
template<int nVars,  int nIneqCon>
template<typename invMatDer>
bool FixedSizeEigenQpSolver<nVars, nIneqCon>::add_constraint(
    Eigen::Matrix<double,nVars,nVars> & R, Eigen::MatrixBase<invMatDer> & Hessian_factor_inv,
    Eigen::Matrix<double,nVars,1> & d,
    int& iq, double& R_norm)
{
...
template<int nVars,  int nIneqCon>
template<typename invMatDer>
void FixedSizeEigenQpSolver<nVars, nIneqCon>::delete_constraint(
    Eigen::Matrix<double,nVars,nVars>& R, Eigen::MatrixBase<invMatDer>& Hessian_factor_inv,
    Eigen::Matrix<int,nIneqCon+0,1> & A, Eigen::Matrix<double,nIneqCon+0,1> & u,
    int p, int& iq, int l)
{
...
#endif /* FIXEDSIZEEIGENQPSOLVER_HH_ */
