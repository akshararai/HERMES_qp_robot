...
const bool RtEiquadprog<nVars, nEqCon, nIneqCon>::must_have_equality_constraints_;
...
template<int nVars, int nEqCon, int nIneqCon>
bool RtEiquadprog<nVars, nEqCon, nIneqCon>::solve()
{
...
//  }
  ...
  solver_return_ = solve_quadprog(BaseClass::objectiveQuadPart(),
...
template<int nVars, int nEqCon, int nIneqCon>
bool RtEiquadprog<nVars, nEqCon, nIneqCon>::isSolved() const
{
  return solver_return_ != std::numeric_limits<double>::infinity();
}

template<int nVars, int nEqCon, int nIneqCon>
void RtEiquadprog<nVars, nEqCon, nIneqCon>::reset()
{
  BaseClass::reset();
  RtMatrixXUtils::setZero(Hessian_factor_inv_, nVars, nVars);
}

template<int nVars, int nEqCon, int nIneqCon>
void RtEiquadprog<nVars, nEqCon, nIneqCon>::reset(int dim_qp, int num_eq, int num_ineq)
{
...
bool RtEiquadprog<nVars, nEqCon, nIneqCon>::add_constraint(typename RtMatrixX< nVars, nVars>::d & R,
      typename RtMatrixX< nVars, nVars>::d & J, typename RtVectorX< nVars>::d & d, int& iq, double& R_norm)
{
int n=J.rows();
#ifdef TRACE_SOLVER
...
void RtEiquadprog<nVars, nEqCon, nIneqCon>::delete_constraint(typename RtMatrixX< nVars, nVars>::d& R,
      typename RtMatrixX< nVars, nVars>::d& J, typename RtVectorX<nIneqCon + nEqCon>::i & A,
      typename RtVectorX< nIneqCon + nEqCon>::d & u, int p, int& iq, int l)
{
...
template<int nVars, int nEqCon, int nIneqCon>
double RtEiquadprog<nVars, nEqCon, nIneqCon>::solve_quadprog(const typename RtMatrixX< nVars, nVars>::d & Hess, const typename RtVectorX< nVars>::d & g0,
                             ...
                             typename RtVectorX< nVars>::d & x)
{
  int i, j, k, l; /* indices */
    ...
    typename RtVectorX<nIneqCon+nEqCon>::d s, r, u;
    RtVectorXUtils::resize(s, p+m);
    RtVectorXUtils::resize(r, p+m);
    RtVectorXUtils::resize(u, p+m);
    typename RtVectorX<nVars+nEqCon>::d u_old;
    RtVectorXUtils::resize(u_old, p+n);
    typename RtVectorX<nVars>::d z, d, np, x_old;
    RtVectorXUtils::resize(z, n);
    RtVectorXUtils::resize(d, n);
    RtVectorXUtils::resize(np, n);
    RtVectorXUtils::resize(x_old, n);
    double f_value, psi, c1, c2, sum, ss, R_norm;
    const double inf = std::numeric_limits<double>::infinity();
    double t, t1, t2; /* t is the step length, which is the minimum of the partial step length t1
      * and the full step length t2 */
    typename RtVectorX<nIneqCon+nEqCon>::i A, A_old, iai, iaexcl;
    RtVectorXUtils::resize(A, p+m);
    RtVectorXUtils::resize(A_old, p+m);
    RtVectorXUtils::resize(iai, p+m);
    RtVectorXUtils::resize(iaexcl, p+m);
    int q;
    int iq, iter = 0;
    ...
    RtVectorXUtils::setZero(d, n);
    RtMatrixXUtils::setZero(R, n, n);
    ...
    if(!is_inverse_provided_)
    {
      RtMatrixXUtils::setIdentity(Hessian_factor_inv_, n);
...
//    std::cout << "hess*inv norm: " << (Hess*Hessian_factor_inv_*Hessian_factor_inv_.transpose() - Eigen::MatrixXd::Identity(Hess.rows(), Hess.cols())) << std::endl;
  ...
  #ifdef TRACE_SOLVER
   print_matrix("Hessian_factor_inv_", Hessian_factor_inv_, n);
  #endif
   ...
   if(is_inverse_provided_)
   {
     x = Hessian_factor_inv_*Hessian_factor_inv_.transpose()*g0;
   }
   else
   {
     x = chol_.solve(g0);
    ...
    iq = 0;
          ...
          {
      np = CE.row(i);
      compute_d(d, Hessian_factor_inv_, np);
                  update_z(z, Hessian_factor_inv_, d,  iq);
                  update_r(R, r, d,  iq);
  #ifdef TRACE_SOLVER
                  print_matrix("R", R, iq);
                  print_vector("z", z, n);
      ...
      if (!add_constraint(R, Hessian_factor_inv_, d, iq, R_norm))
      {
  ...
  if(iq >= Hessian_factor_inv_.cols())
  {
    ...
    if (t2 >= inf)
    {
      ...
      u(iq) += t;
      iai(l) = l;
      delete_constraint(R, Hessian_factor_inv_, A, u, p, iq, l);
  #ifdef TRACE_SOLVER
    ...
    if (t == t2)
    {
      ...
      /* add constraint ip to the active set*/
                  ...
                  {
                          iaexcl(ip) = 0;
                          delete_constraint(R, Hessian_factor_inv_, A, u, p, iq, ip);
  #ifdef TRACE_SOLVER
        print_matrix("R", R, n);
