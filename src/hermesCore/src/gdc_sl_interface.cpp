                                       ...
                                       std::vector<hermes_communication_tools::GDCState>& gdc_state)
{
  ...
  for(int j=0; j<(int)gdc_state.size(); ++j)
  {
    ...
    //////////////// AKSHARA : HACK TO ADD FEEDFORWARD TORQUES TO THE DESIRED TORQUE
    temp_command = temp_command + joint_trans_torques_[dof_num].desired_offset_;
   ...
   //if(dof_num == 20) std::cout << "real desired " << temp_command << "desired" << joint_trans_torques_[dof_num].desired_offset_ << std::endl;
    ///TODO do more checks!!
...
bool GDCSLInterface::read_sensor_translation()
{
  ...
  for (int i=1; i<= n_dofs; ++i)
  {
    ...
    err = fscanf(my_file,"%lf %lf %lf %lf %lf %lf %lf",
                 ...
                 &joint_trans_torques_[i].desired_offset_);
    std::cout << "dof_number " << i << "offset " <<  joint_trans_torques_[i].desired_offset_ << std::endl;
    if(err != 7)
    {
