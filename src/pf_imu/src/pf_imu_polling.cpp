...
void pf_IMU::read_pf_imu()
{
  ...
  imu_state = waiting_for_header;

  unsigned int header_index = 0;
  //the main reading loop
  while(imu_state != disconnected)
  {

    unsigned char buff[36];
    usleep(10);

    if(imu_state == waiting_for_header){
    ...
    if (imu_state == getting_data){
      ...
      if (msg_size == 0){
        imu_state = publishing;
      }

    }

    if (imu_state == publishing){
      time1 = rt_timer_read();
      RTIME delta_time = time1-time2;
      time2 = time1;

      // if (delta_time > 10e9/100){
      //   printf("BAD TIMING, SCRUBBING BUFF");
      //   static char scrub[1000];
      //   ssize_t response = read(fd_, scrub, 1000);
      // }
      ...
      if(streaming_)
      {
        ...
        */
        ...
        log.time = double(delta_time)/10e9;
    ...
    // if (rt_timer_read() - time1 > 10e9){
    //   imu_state = disconnected;
    // }
  }

  std::cout<<"!!! IMU DISCONNECTED !!!"<<std::endl;
  std::cout<<double(rt_timer_read() - time1)/10e9<<" seconds since last read"<<std::endl;
...
{
  std::cout<<"getting mutex"<<std::endl;
  rt_mutex_acquire(&mutex_,TM_INFINITE);
